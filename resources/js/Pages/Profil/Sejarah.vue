...
<template>
    ...
    <Head :title="`Sejarah ${$page.props.settings?.sebutan_wilayah || 'Kelurahan'}`" />
    ...
    <MainLayout>
        ...
        <div class="bg-blue-900 py-16 relative overflow-hidden">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
                <h1 class="text-4xl md:text-6xl font-black text-white tracking-tight mb-4">Sejarah {{
                    $page.props.settings?.sebutan_wilayah || 'Kelurahan' }}</h1>
                <p class="text-blue-100 text-lg max-w-2xl mx-auto font-medium">
                    Menelusuri warisan budaya dan perkembangan {{ $page.props.settings?.sebutan_wilayah || 'Kelurahan'
                    }} {{ $page.props.settings?.nama_wilayah || '[Nama Wilayah]' }} dari masa ke masa.
                </p>
            </div>
        </div>
        ...
        <div class="py-24 bg-slate-50 relative overflow-hidden">
            <!-- Background Shape -->
            <div
                class="absolute top-0 right-0 -translate-y-1/2 translate-x-1/2 w-[600px] h-[600px] bg-blue-500/5 rounded-full blur-[100px] pointer-events-none">
            </div>
            ...
            <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative">
                ...
                <div v-if="intro" class="mb-32 relative group">
                    <div
                        class="absolute -inset-2 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-[3rem] blur opacity-10 group-hover:opacity-20 transition duration-500">
                    </div>
                    <div
                        class="relative bg-white/80 backdrop-blur-xl p-10 md:p-14 rounded-[2.5rem] border border-white shadow-2xl">
                ...
                <div v-if="timelines && timelines.length > 0" class="relative">
                    ...
                    <div
                        class="absolute left-1/2 transform -translate-x-1/2 w-1.5 h-full bg-slate-200 hidden md:block rounded-full">
                        <div
                            class="absolute inset-0 bg-gradient-to-b from-blue-600 via-indigo-500 to-emerald-500 opacity-20 rounded-full">
                        </div>
                    </div>
                    ...
                    <div class="space-y-24">
                        <div v-for="(t, index) in timelines" :key="t.id"
                            ...
                            <div
                                class="absolute left-1/2 transform -translate-x-1/2 hidden md:flex items-center justify-center z-10 transition-transform duration-500 group-hover:scale-110">
                                <div class="w-10 h-10 rounded-full bg-white shadow-xl border-4 flex items-center justify-center"
                                    :class="getColors(index).border">
                                    ...
                                    <div class="absolute inset-0 rounded-full animate-ping opacity-20"
                                        :class="getColors(index).dot"></div>
                                </div>
                            </div>

                            <!-- Year Label (Desktop side) -->
                            <div class="hidden md:flex flex-col items-center w-[45%]"
                                :class="index % 2 === 0 ? 'items-end pr-12' : 'items-start pl-12'">
                                <div class="px-6 py-2 rounded-2xl bg-white shadow-lg border-2 font-black text-xl tracking-tighter"
                                    :class="[getColors(index).border, getColors(index).text]">
                                    {{ t.year }}
                                </div>
                            </div>
                            ...
                            <div class="w-full md:w-[45%]">
                                <div class="relative overflow-hidden bg-white p-8 md:p-10 rounded-[2.5rem] shadow-2xl transition-all duration-500 border border-slate-100 group-hover:-translate-y-2 group-hover:shadow-[0_20px_50px_rgba(0,0,0,0.1)]"
                                    ...
                                    <div class="absolute top-0 bottom-0 w-2 transition-all duration-500 group-hover:w-3"
                                        :class="[index % 2 === 0 ? 'right-0' : 'left-0', getColors(index).accent]">
                                    </div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <h3
                                            class="text-2xl font-black text-slate-900 mb-6 leading-tight group-hover:translate-x-2 transition-transform duration-500">
                                            {{ t.title }}
                                        </h3>
                ...
                <div v-else-if="!intro"
                    class="text-center py-20 bg-white rounded-[4rem] border-4 border-dashed border-slate-100">
