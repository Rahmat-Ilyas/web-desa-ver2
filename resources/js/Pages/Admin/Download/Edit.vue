...
const form = useForm({
    _method: 'PUT',
    judul: props.download.judul,
    kategori: props.download.kategori,
    file: null,
    deskripsi: props.download.deskripsi || '',
});
        ...
        <div class="max-w-[1000px] mx-auto px-4">
            ...
            <div class="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    ...
                    <h1 class="text-4xl font-black text-slate-900 tracking-tight">Edit Dokumen</h1>
                    <p class="text-slate-500 font-bold text-sm mt-2">Perbarui informasi atau ganti file dokumen.</p>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="space-y-8">
                        ...
                        <div class="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100">
                            ...
                            <!-- Current File Info if available and no new file selected -->
                            <div v-if="!filePreview && download.file_path"
                                class="bg-slate-50 rounded-2xl p-4 mb-4 border border-slate-200">
                                <p class="text-[10px] font-black text-slate-400 uppercase mb-2">File Saat Ini:</p>
                                <div class="flex items-center gap-3">
                                    <div
                                        class="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-slate-500 shadow-sm">
                                        <i :class="['fas text-lg', getFileIcon(download.file_path)]"></i>
                                    </div>
                                    <div class="overflow-hidden">
                                        <a :href="'/storage/' + download.file_path" target="_blank"
                                            class="text-xs font-bold text-blue-600 hover:underline truncate w-full block">Download
                                            / Lihat</a>
                                        <p class="text-[10px] font-bold text-slate-400 uppercase">{{ download.type }} •
                                            {{ download.size }}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div v-if="filePreview"
                                class="bg-blue-50 rounded-2xl p-4 mb-4 relative group border border-blue-100">
                                <p class="text-[10px] font-black text-blue-400 uppercase mb-2">Akan Diganti Dengan:</p>
                                <div class="flex items-center gap-3">
                                    <div
                            ...
                            <label v-else
                                ...
                                <span class="text-xs font-bold text-slate-400 group-hover/upload:text-slate-600">
                                    {{ download.file_path ? 'Ganti File' : 'Upload File' }}
                                </span>
                        ...
                        <div
                            ...
                            <h3 class="text-lg font-black mb-2 relative z-10 uppercase tracking-tight">Simpan Perubahan?
                            </h3>
                            <p class="text-slate-400 font-bold text-xs mb-6 leading-relaxed relative z-10">Pastikan data
                                yang diubah sudah benar.</p>
                            <button type="submit" :disabled="form.processing"
                                ...
                                <span v-else class="flex items-center gap-2">
                                    UPDATE DATA <i
                                        class="fas fa-save group-hover/btn:scale-110 transition-transform"></i>
                                </span>
                            </button>
                        </div>
                    </div>
