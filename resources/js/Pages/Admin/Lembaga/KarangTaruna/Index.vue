...
const title = 'Pengurus Karang Taruna';
...
const profileForm = useForm({
    profil_karang_taruna: props.profile || '',
    tugas_karang_taruna: props.tasks && props.tasks.length > 0 ? props.tasks : [
...
const addTask = () => {
    profileForm.tugas_karang_taruna.push("");
};

const removeTask = (index) => {
    if (profileForm.tugas_karang_taruna.length > 1) {
        profileForm.tugas_karang_taruna.splice(index, 1);
    }
};

const submitProfile = () => {
    profileForm.put(route('admin.karang-taruna.profile.update'), {
        preserveScroll: true,
        onSuccess: () => showNotification('Profil & Tugas Karang Taruna berhasil diperbarui'),
    });
};
...
const openEditModal = (item) => {
    ...
    form.bidang = item.bidang || 'Pengurus Harian';
...
const submit = () => {
    if (editingItem.value) {
        router.post(route('admin.karang-taruna.update', editingItem.value.id), {
            _method: 'PUT',
            ...form.data(),
        }, {
            forceFormData: true,
        ...
        form.post(route('admin.karang-taruna.store'), {
            onSuccess: () => closeModal(),
        });
    }
};
...
const confirmDelete = () => {
    if (itemToDelete.value) {
        router.delete(route('admin.karang-taruna.destroy', itemToDelete.value.id), {
            onSuccess: () => {
                                ...
                                <div class="pt-8 border-t border-slate-50">
                                    ...
                                    <div class="space-y-3 max-h-[400px] overflow-y-auto pr-2 pb-10 custom-scrollbar">
                                        <transition-group enter-active-class="transition duration-300 ease-out"
                                            ...
                                            <div v-for="(task, index) in profileForm.tugas_karang_taruna" :key="index"
                                                class="flex gap-2 items-start group">
                                                <div class="flex-grow relative">
                                                    <input v-model="profileForm.tugas_karang_taruna[index]" type="text"
                                                        ...
                                                        placeholder="Tuliskan tugas/fokus...">
                                                    <div
