...
<template>
    <AdminLayout>
        ...
        <div class="mb-8 flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-black text-slate-900">Struktur Organisasi</h1>
                <p class="text-slate-500 font-bold text-sm mt-1">Kelola jajaran pimpinan dan staf Kelurahan Ujung
                    Sabbang.</p>
            </div>
            <button @click="openCreateModal"
                class="bg-blue-600 text-white px-6 py-3 rounded-xl font-black text-sm hover:bg-blue-700 transition-all shadow-lg shadow-blue-600/20 flex items-center gap-2">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/50 border-b border-slate-100">
                        <th class="px-6 py-5 text-xs font-black text-slate-500 uppercase tracking-widest">Pejabat/Staf
                        </th>
                        <th class="px-6 py-5 text-xs font-black text-slate-500 uppercase tracking-widest">Jabatan</th>
                        <th class="px-6 py-5 text-xs font-black text-slate-500 uppercase tracking-widest text-center">
                            Tingkat</th>
                        <th class="px-6 py-5 text-xs font-black text-slate-500 uppercase tracking-widest">Atasan
                            Langsung</th>
                        <th class="px-6 py-5 text-xs font-black text-slate-500 uppercase tracking-widest text-right">
                            Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-50">
                    <tr v-for="item in structures" :key="item.id" class="hover:bg-slate-50/50 transition-colors">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-wider">NIP.
                                        {{ item.nip || '-' }}</p>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4">
                            <div v-if="item.parent" class="flex items-center gap-2">
                                ...
                                <div>
                                    <p class="font-bold text-slate-700 text-sm">{{ item.parent.name }}</p>
                                    <p class="text-[10px] text-slate-400 font-medium">{{ item.parent.position }}</p>
                                </div>
                            </div>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <button @click="openEditModal(item)"
                                    class="w-9 h-9 flex items-center justify-center bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-600 hover:text-white transition-all">
                                    <i class="fas fa-edit text-xs"></i>
                                </button>
                                <button @click="openDeleteModal(item)"
                                    class="w-9 h-9 flex items-center justify-center bg-rose-50 text-rose-600 rounded-lg hover:bg-rose-600 hover:text-white transition-all">
        ...
        <transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0 scale-95"
            ...
            <div v-if="isModalOpen"
                ...
                <div class="bg-white rounded-[2rem] shadow-2xl w-full max-w-2xl overflow-hidden border border-white/20">
                    <div class="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                        <h3 class="text-lg font-black text-slate-900">{{ editingItem ? 'Edit Anggota' : 'Tambah Anggota'
                            }}</h3>
                    ...
                    <form @submit.prevent="submit" class="p-8 space-y-8 max-h-[85vh] overflow-y-auto custom-scrollbar">
                        ...
                        <div class="flex flex-col lg:flex-row gap-10">
                            ...
                            <div class="lg:w-5/12 space-y-6">
                                ...
                                <div class="relative group">
                                    ...
                                    <!-- Photo Error Display -->
                                    <p v-if="errors.photo || photoError" class="mt-1 text-rose-500 text-[10px] font-bold">{{
                                        errors.photo?.[0] || photoError }}</p>

                                    <p class="text-[10px] text-center text-slate-400 font-medium mt-2">Format: JPG/PNG,
                                ...
                                <div class="space-y-4">
                                    <div
                                        ...
                                        <label
                                            ...
                                            Lengkap</label>
                                        <input v-model="form.name" type="text" placeholder="Contoh: Budi Santoso, S.Sos"
                                    ...
                                    <div class="space-y-4">
                                        ...
                                        <div>
                                            ...
                                            <div class="grid grid-cols-2 gap-3">
                                                ...
                                                <button type="button" @click="form.level = 2"
                                                    ...
                                                    <p class="text-[10px] text-slate-400 leading-tight">Wakil Pimpinan
                                                    </p>
                                                </button>
                                                ...
                                                <button type="button" @click="form.level = 3"
                                                    ...
                                                    <p class="text-[10px] text-slate-400 leading-tight">Kepala Seksi</p>
                                                </button>
                                                ...
                                                <button type="button" @click="form.level = 4"
                                                    ...
                                                    <p class="text-[10px] text-slate-400 leading-tight">Anggota Umum</p>
                                                </button>
                                            </div>
                                        ...
                                        <div>
                                            ...
                                            <div class="relative">
                                                <select v-model="form.parent_id"
                                                    ...
                                                    <option :value="null">-- Tidak Ada / Langsung ke Lurah --</option>
                                        ...
                                        <div>
                                            ...
                                            <div class="flex items-center gap-2">
                                                ...
                                                <button type="button" @click="form.order++"
                                                    class="w-8 h-8 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold transition-colors">+</button>
                                                <span class="text-[10px] text-slate-400 ml-2">Semakin kecil, semakin di
                                                    kiri/atas.</span>
                                                <p v-if="errors.order" class="text-rose-500 text-[10px] font-bold mt-1">
                                                    {{ errors.order?.[0] }}
                                                </p>
                                            </div>
                                        </div>
